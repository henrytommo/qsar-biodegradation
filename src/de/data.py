...
DATA_PATH = Path(__file__).resolve().parents[2] / "data" / "biodeg.csv"
...
FEATURE_NAMES = [f.replace("[", "(").replace("]", ")") for f in FEATURE_DICT.keys()]

BINARY_FEATS = ["B01(C-Br)", "B03(C-Cl)", "B04(C-Br)"]


def load_data(path=DATA_PATH):
    """Load the raw biodeg dataset, returning (X, y) with y encoded as RB=1 / NRB=0."""
    data = pd.read_csv(path, header=None, names=FEATURE_NAMES, sep=";")
    X = data.iloc[:, :-1]
    y = np.where(data.iloc[:, -1] == "RB", 1, 0)
    return X, y
